...
//Initialize Shaders
if (!initShaders(gl, ORBITVSHADER_SOUCE, FSHADER_SOURCE)) {
...
function initVertexBuffers(gl){
  ...
  for (j = 0; j < SPHERE_DIV; j++) {
    for (i = 0; i < SPHERE_DIV; i++) {
      p1 = j * (SPHERE_DIV+1) + i;
      p2 = p1 + (SPHERE_DIV+1);
      let nlong = (2*Math.PI/SPHERE_DIV)*i;
      let nlat = (Math.PI/SPEHERE_DIV)*j;
      ...
      textureCoords[p1*4 + 0] = i/nlong + 1;
      textureCoords[p1*4 + 1] = j/nlat + 1;

      textureCoords[p1*4 + 2] = i/nlong;
      textureCoords[p1*4 + 3] = i/nlat;
    }
  }
...
function initTextures(gl, planets){
  planets.forEach(planet => {
    planet.texture = gl.createTexture();
    let image = new Image();
    image.onload = function(){ loadTexture(gl, planet.texture, image);};
...
function render_PLANET(gl,n,planet,u_modelMatrix,u_NormalMatrix,vertexBuffer,a_Position,u_Sampler,textureBuffer){

    gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffer);
    gl.vertexAttribPointer(a_Position,vertexBuffer.num,vertexBuffer.type,false,0,0);
    gl.enableVertexAttribArray(a_Position);

    gl.vertexAttribPointer(a_Normal,vertexBuffer.num,vertexBuffer.type,false,0,0);
    gl.enableVertexAttribArray(a_Normal);
    gl.bindBuffer(gl.ARRAY_BUFFER, null);

    gl.bindBUffer(gl.ARRAY_BUFFER,textureBuffer);
    gl.vertexAttribPointer(a_TexCoord,textureBuffer.num,textureBuffer.type,false,0,0);
...
let tick = function(){
  ...
  orbits.forEach(orbit => {
    ...
    if (!initShaders(gl, ORBIT_VSHADER_SOUCE, ORBIT_FSHADER_SOURCE)) {
      console.log('Failed to initialize shaders.');
    }
  ...
  planets.forEach(planet => {
    ...
    if (!initShaders(gl, PLANET_VSHADER_SOUCE, PLANET_FSHADER_SOURCE)) {
      console.log('Failed to initialize shaders.');
    }
